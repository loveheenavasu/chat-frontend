...
import { Box, Button, Card, Checkbox, FormControl, Input, Select, Table, Text, Th, Thead, Tr } from '@chakra-ui/react';
...
const DynamicForm = () => {
    const [fields, setFields] = useState([{ type: 'text', name: '', label: '', isRequired: false }]);
    const [selectedIndexes, setSelectedIndexes] = useState<string[]>([]);

    const addField = () => {
        setFields([...fields, { type: 'text', name: '', label: '', isRequired: false }]);
    };
    ...
    const requiredCheckBox = (index: number, event: ChangeEvent<HTMLInputElement>) => {
    ...
    const handleSubmit = async (e: any) => {
        ...
        const selectedFields = selectedIndexes.map((index) => {
            ...
            return { type: field?.type || '', name: field?.value || '', label: field?.label || '' };
        });
    ...
    return (
        <Box>
            <Box display={'flex'} alignItems={'center'} justifyContent={'space-between'} mb={5}>
                <Text fontSize={25} fontWeight={'700'}>Lead Generation Form</Text>
                    ...
                    <Box>
                        ...
                        <Box mt={5}>
                            ...
                            {selectedIndexes.map((selectedValue) => {
                                const selectedItem = staticInputFields.find(item => item.value === selectedValue);
                                return (
                                    <Box display={'flex'}
                                        gap={4}
                                        mb={4}
                                    ...
                                    >
                                        <Box textColor={'gray'} fontWeight={500}>
                                            <Card p={'10px 70px'} textColor={'gray.500'} bg={'#e2e8f0'}>
                        ...
                        {fields.map((field, index) => (
                            <Box key={index} mb={4}>
                                <Box display={'flex'} justifyContent={'space-around'} alignItems={'center'}>
                                    <Box>
                                        <label style={{ marginBottom: '20px', fontSize: 'small', fontWeight: '500' }}
                                    ...
                                    <Box w={'180px'}>
                                        <label
                                            style={{ marginBottom: '20px', fontSize: 'small', fontWeight: '500' }}
                                        >Field Name</label>
                                        <Input
                                            type='text'
                                            value={field.name}
                                    ...
                                    <Box w={'180px'}>
                                        <label style={{ marginBottom: '20px', fontSize: 'small', fontWeight: '500' }}>Label</label>
                                        <Input
                                            type='text'
                                            value={field.label}
                                            name="label"
                                    ...
                                    <Box display={'flex'}
                                        ...
                                        mb={4}
                                    >
                                        <label style={{ marginBottom: '8px', fontSize: 'small', fontWeight: '500' }} >Required</label>
                                        <Checkbox
                                    ...
                                    <Box mt={4}>
                                        <RxCross2
                                            color='white'
                                            style={{ background: 'red', borderRadius: '4px', cursor: 'pointer' }}
