...
const DynamicForm = () => {
    ...
    const [selectedIndexes, setSelectedIndexes] = useState<string[]>(["name", "email", "phone"]);
    const [newField, setNewField] = useState({ type: "text", label: "", name: '' });
    ...
    const fetchData = async () => {
        try {
            const response = await axiosInstance.get(`user/form/?documentId=${documentId}`);
            if (response.status === 200) {
                const result = response.data.fields;
                console.log(result, 'resultsss')
                const data = result.filter((item: any) => !staticInputFields.some(staticField => staticField.value === item.name))
                setItems(data);
                setIsDataSubmitted(true);
                const apiStaticFields = result?.filter((field: any) =>
                    staticInputFields.some(staticField => staticField.value === field.name)
                );
    ...
    const handleNewFieldChange = (event: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    const handleCheckboxChange = (value: string) => {
        ...
        setItems(prevItems => prevItems.filter(item => !staticInputFields.some(staticField => staticField.value === item.name)))
    };
    ...
    return (
        <>
            <Box>
                ...
                <CardContainer border="1px solid #e2e8f0" padding="20px" boxShadow="md" borderRadius="10px" height="auto">
                    <FormControl as="form" onSubmit={handleSubmit} mb={4}>
                        <Text textAlign="center" my="3" fontSize={16} fontWeight={400} mb={4}>
                            Select any Field from here or add any custom field by clicking on + button
                        </Text>
                            ...
                            {fields.map((field, index) => (
                                <Box mb={4}>
                                    ...
                                    >
                                        ...
                                        <Button colorScheme="red" position="absolute" top="10px" right="10px" onClick={() => removeField(index)}>
                                    ...
                                    <Box mb={4}>
                                        ...
                                        >
                                            <Box position="absolute" top="10px" right="10px">
                                                <Button
                                                    _hover={disabledFields[index] ? { bg: "#363535" } : { bg: '#2a7ac4 ' }}
                                                    bg={disabledFields[index] ? "black" : "#3182CE"}
                                                    textColor="white"
                                                    onClick={() => handleEdit(index)}
                                                    mr={2}
                                                >
                                                    {disabledFields[index] ? <EditIcon /> : <TiTick size={'20px'} />}
                                                </Button>
